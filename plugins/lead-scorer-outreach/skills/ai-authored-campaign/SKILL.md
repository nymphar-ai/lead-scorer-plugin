...
## Steps
1. **Senders.** `list_sender_accounts` — confirm which account will send; stop if none is connected.
2. **Create.** `create_campaign` (draft) + `add_leads_to_campaign` for the list.
