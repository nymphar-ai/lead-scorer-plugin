...
4. **List quality.** `get_leads_from_list` on the campaign list: what share has a real, individual, corporate address? Flag role addresses (contact@, info@, sales@), free-mail addresses, and leads with no email at all. Role and catch-all addresses are the main bounce source, and bounces above ~2% are a list problem, never a copy problem. For requested verification of existing emails, estimate with `verify_lead_emails` (lead_id, `dry_run: true`), explain the 1-credit cost per successful lead verification, and respect approval and confirmation rules before executing. Only accessible emails are checked; failures are refunded. Read the saved status, provider and date with `get_lead`. Do not rerun a verification just to read results or call contact discovery for an already known address.
