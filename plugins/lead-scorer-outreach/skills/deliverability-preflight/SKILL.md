...
## Steps
1. **Sender.** `list_sender_accounts` — which account sends, is it healthy, how long has it been connected? Report if the campaign would send from my primary business domain. **That is the one thing to avoid**: cold volume belongs on a dedicated sending subdomain or a separate domain.
