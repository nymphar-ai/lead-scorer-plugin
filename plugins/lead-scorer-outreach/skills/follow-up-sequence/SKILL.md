...
## Steps
1. `get_campaign` + `get_campaign_authoring_context` — read the first touch already written for each lead. Follow-ups must not repeat its argument.
...
4. Push with `write_campaign_drafts`; review with `list_campaign_actions` and fix with `update_campaign_action_draft`.
